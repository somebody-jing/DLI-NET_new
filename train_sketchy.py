import torch
from dataset.sketchy_dataset import TrainDataset, TestPhDataset, TestSkDataset
...
from options.sketchy_options import TrainOptions
import itertools
...
def train():
    ...
    train_dataset = TrainDataset(opt.ph_train_root, opt.sk_train_root, opt.ph_train_txt, opt.dict_path)
    ...
    for i in range(epoch + 1, opt.epoch + 1):
        ...
        # t0 = time.time()
        for ph, sk, label in tqdm_batch:
            ph, sk, label = ph.cuda(), sk.cuda(), label.cuda()
            ph_fea,ph_cls = ph_encoder(ph)
            sk_fea,sk_cls = sk_encoder(sk)
            if opt.self_interaction:
